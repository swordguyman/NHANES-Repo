...
datasetcorrect=dataset[!is.na(dataset)]
 ...
 McN=Mclust(boots,G=1, prior=priorControl(),initialization=list(hcPairs=hc("E",boots)))
 BIC[i,b,1]=-McN$bic
 ...
 McLN=Mclust(lnboots,G=1, prior=priorControl(),initialization=list(hcPairs=hc("E",boots)))
...
stddevs[i,b,2]= sqrt(McLN$parameters$variance$sigmasq)
 ...
 McE=Mclust(lnboots,G=2,modelNames="E", prior=priorControl(),initialization=list(hcPairs=hc("E",boots)))
 ...
 McV=Mclust(lnboots,G=2,modelNames="V" ,prior=priorControl(),initialization=list(hcPairs=hc("E",boots)))
...
Mc3E=Mclust(lnboots,G=3,modelNames="E", prior=priorControl(),initialization=list(hcPairs=hc("E",boots)))
...
Mc3V = Mclust(lnboots,G=3,modelNames="V",prior=priorControl(),initialization= list(hcPairs=hc("E",boots)))
...
nBDcols = 62
 ...
 saveWorkbook(BDbootfull, "BigDatabootfull_output_glu_1998.xlsx")  
 print(proc.time()-pmt)
